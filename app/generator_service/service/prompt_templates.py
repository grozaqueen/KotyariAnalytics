def make_prompt(context: dict) -> list[dict]:
    ...
    system = (
        ...
        "Не копируй примеры — пиши оригинально на их манер. Избегай фактических ошибок. Избегай рекламу, говори только по сути"
        ...
        "Сначала сгенерируй заголовок примерно в 5-6 слов, пустьь он будет в первом абзаце, а вторым абзацемсгенерируй сам пост."
        "Нужно понимать, что пишешь действительно по теме, если вдруг в примерах для манера не те данные вообще, не стоит их использовать. Пойми, про что именно тебя просят написать и что ты написал. Если увидишь несоответствие, забудь о высланном контексте, пиши по запросу."
    )
